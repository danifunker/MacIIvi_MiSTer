-- tb_basic_chk2_packed_state.vhd
-- Exact packed-state reproducer for BASIC/CHK2.W/0001 record=0 group=4
-- subcase=0. This keeps the decoded opcode patch (02D0 0800), uses the base
-- low-memory bounds at $0000/$0002, and arms T1 through the reconstructed
-- user-mode frame that cputest would execute.

library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

entity tb_basic_chk2_packed_state is
end entity;

architecture behavior of tb_basic_chk2_packed_state is
    ...
    constant FRAME_ADDR  : integer := ISP_VALUE - 12;
...
begin
    ...
    test: process
        ...
        begin
            ...
            low_mem(RESULT_ADDR / 2 + 2) := x"0000";
            ...
            high_mem(idx) := x"2A3C";
            high_mem(idx + 1) := x"8000";
            high_mem(idx + 2) := x"8080";
            idx := idx + 3;
            ...
            high_mem(idx) := x"33FC";
            high_mem(idx + 1) := x"004F";
            high_mem(idx + 2) := x"0000";
            high_mem(idx + 3) := x"0002";
            idx := idx + 4;
            ...
            high_mem((FRAME_START - HIGH_BASE) / 2) := x"8000";
            ...
            high_mem(idx + 6) := x"42F9";
            high_mem(idx + 7) := x"0000";
            high_mem(idx + 8) := std_logic_vector(to_unsigned(RESULT_ADDR + 2, 16));
            high_mem(idx + 9) := x"60FE";
        end procedure;
    ...
    begin
        report "=== MC68030 BASIC packed CHK2.W state reproducer ===" severity note;

        init_case;
        run_case;

        if mem_read(RESULT_ADDR) = MARK_TRACE then
            report "PASS: packed CHK2.W case took trace" severity note;
            pass_count := pass_count + 1;
        elsif mem_read(RESULT_ADDR) = MARK_CHK then
            report "FAIL: packed CHK2.W case took CHK exception" severity error;
            fail_count := fail_count + 1;
        else
            report "FAIL: packed CHK2.W case missed trace" severity error;
            fail_count := fail_count + 1;
        end if;

        load_frame(FRAME_ADDR);

        if frame_sr = x"8000" then
            report "PASS: packed CHK2.W saved SR" severity note;
            pass_count := pass_count + 1;
        else
            report "FAIL: packed CHK2.W saved SR mismatch" severity error;
            fail_count := fail_count + 1;
        end if;

        if frame_fv = x"2024" then
            report "PASS: packed CHK2.W format/vector" severity note;
            pass_count := pass_count + 1;
        else
            report "FAIL: packed CHK2.W format/vector mismatch" severity error;
            fail_count := fail_count + 1;
        end if;

        if frame_pc = x"42050004" then
            report "PASS: packed CHK2.W stacked PC" severity note;
            pass_count := pass_count + 1;
        else
            report "FAIL: packed CHK2.W stacked PC mismatch" severity error;
            fail_count := fail_count + 1;
        end if;

        if frame_ia = x"42050000" then
            report "PASS: packed CHK2.W instruction address" severity note;
            pass_count := pass_count + 1;
        else
            report "FAIL: packed CHK2.W instruction address mismatch" severity error;
            fail_count := fail_count + 1;
        end if;

        report "packed CHK2.W tests: " & integer'image(pass_count) & " PASSED, " &

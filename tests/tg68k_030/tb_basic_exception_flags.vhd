-- tb_basic_exception_flags.vhd
-- Maintained MC68030 BASIC-style exception flag coverage for the CHK and DIV
-- cases that still show up in hardware cputest/basic.

library ieee;
use ieee.std_logic_1164.all;
use ieee.numeric_std.all;

entity tb_basic_exception_flags is
end entity;

architecture behavior of tb_basic_exception_flags is
...
begin
    ...
    begin
        report "=== MC68030 BASIC Exception Flag Coverage ===" severity note;

        report "=== Test 1: CHK.W user T1 negative destination ===" severity note;
        init_common;
        setup_vector(16#18#, 16#2200#);
        setup_vector(16#24#, 16#2300#);
        ...
        mem(16#100A# / 2) := x"4181";
        mem(16#100C# / 2) := x"4E71";
        run_case;
        check_frame("CHK.W user T1", x"800B", x"018", x"0000100C", x"0000100A");

        report "=== Test 2: CHK.L user T1 negative destination ===" severity note;
        init_common;
        setup_vector(16#18#, 16#2200#);
        setup_vector(16#24#, 16#2300#);
        setup_handler(16#2200#);
        ...
        mem(16#100C# / 2) := x"4101";
        mem(16#100E# / 2) := x"4E71";
        run_case;
        check_frame("CHK.L user T1", x"800B", x"018", x"0000100E", x"0000100C");

        report "=== Test 3: DIVU.W user T1 divide-by-zero ===" severity note;
        init_common;
        ...
        mem(16#100E# / 2) := x"4E71";
        run_case;
        check_frame("DIVU.W user T1", x"8006", x"014", x"0000100E", x"0000100A");

        report "=== Test 4: DIVUL.L user T1 divide-by-zero ===" severity note;
        init_common;
        setup_vector(16#14#, 16#2400#);
        setup_vector(16#24#, 16#2300#);
        setup_handler(16#2400#);
        ...
        mem(16#1008# / 2) := x"46FC";
        mem(16#100A# / 2) := x"8000";
        mem(16#100C# / 2) := x"4C42";
        mem(16#100E# / 2) := x"0000";
        mem(16#1010# / 2) := x"4E71";
        run_case;
        check_frame("DIVUL.L user T1", x"8002", x"014", x"00001010", x"0000100C");

        report "BASIC exception flag tests: " & integer'image(pass_count) & " PASSED, " &
               integer'image(fail_count) & " FAILED" severity note;

        if fail_count = 0 then
            report "ALL BASIC EXCEPTION FLAG TESTS PASSED!" severity note;
        else
            report "BASIC EXCEPTION FLAG TESTS FAILED" severity failure;
        end if;

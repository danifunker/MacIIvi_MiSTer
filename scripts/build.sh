#!/usr/bin/env bash
# Build MacIIvi.rbf via Quartus. Waits for any in-progress quartus to finish.
set -u
...
quartus_sh --flow compile MacIIvi 2>&1 | tee -a "$LOG"
RC=${PIPESTATUS[0]}
rm -f output_files/.compile_in_progress
echo "[$(date)] Compile exit=$RC at $(date)" | tee -a "$LOG"
ls -la output_files/MacIIvi.{sof,rbf} 2>&1 | tee -a "$LOG"
exit $RC

...
> `A`s; `MacIIvi` is the first `M`, before `MSX`). The API returns them case-*sensitively*,
...
| `--seed-mount-rel REL` | — | — | relative path stored in the `.s<N>` file (e.g. `games/MacIIvi/MacIIvi.nvr`) |
...
In this repo, `scripts/deploy_screenshot.sh` is the MacIIvi wrapper: it verifies the

...
hardware: "ao486" sorts among the "A"s; a core named "MacIIvi" is the first "M",
...
def verify(host, port, expect):
    ...
    # partial-word overlap like "MACLC" vs "MacIIvi" -- the old `r in exp` here
...
def main():
    ...
    ap.add_argument("--core", default=os.environ.get("RBF_NAME"),
                    help="core filename to launch, e.g. MacIIvi.rbf (env RBF_NAME)")
    ...
    ap.add_argument("--seed-mount-rel",
                    help="relative path stored in --seed-mount-cfg (e.g. games/MacIIvi/MacIIvi.nvr)")
    ...
    # MacLC instead of MacIIvi). Do NOT press `osd` at the main menu -- that opens a

...
// Here: clk = clk_sys, divide by 8. clk_sys is nominally 32.5 MHz but the
// fractional PLL actually produces 50 MHz x 1165/128 / 14 = 32.5055804 MHz
// (+171.7 ppm; see the STA Clocks table), so cen = 4,063,197.545 Hz.
// (-3.1% vs the real HC05 rate; firmware-relative timing is unaffected, and
// the one-second/RTC period below is corrected in absolute terms. The PLL's
// dedicated 4.194304 MHz outclk2 is NOT usable: the shared VCO can only
// reach +/-0.46% of it.)
// ============================================================================
reg [2:0] clk_div;
...
// Here: count cen pulses (4,063,197.545 Hz ACTUAL — see clock generation
// above; the PLL does not make the nominal 4.0625 MHz). The counter fires on
// the pulse where it REACHES ONESEC_PERIOD and restarts at 0, i.e. every
// ONESEC_PERIOD+1 pulses: 4_063_197+1 = 4,063,198 = 1.000000112 s
// (-0.11 ppm ~= 10 ms/day slow, below a real Mac's RTC crystal tolerance).
// This period feeds the firmware's RTC-seconds ISR AND the per-second host
// notify that advances the Mac's Time global, so it must be correct in
// ABSOLUTE time. History: 22'd4000000 gained ~22 min/day (2026-07-15 clock
// audit); 22'd4062499 assumed a true 32.5 MHz clk_sys and gained +14.8 s/day
// (+171.7 ppm, caught on the 2026-07-16 hardware soak).
`ifdef SIMULATION
localparam ONESEC_PERIOD = 22'd8192;     // ~2ms (fast for simulation)
`else
localparam ONESEC_PERIOD = 22'd4063197;  // 1.000 s of ACTUAL cen pulses
`endif
